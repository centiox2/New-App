...
import type { GlobalSearchResult, QuickSearchResult } from '../../shared/ipc-types'

const RESULT_LIMIT = 40
const GLOBAL_RESULT_LIMIT = 60

/** Every table search:quickSearch and search:globalSearch both query, for one client. */
async function searchWithinClient(clientId: string, pattern: string): Promise<QuickSearchResult[]> {
  const db = getDb()
...
export function registerSearchHandlers(): void {
  ipcMain.handle(
    ...
    async (_e, args: { clientId: string; query: string }): Promise<QuickSearchResult[]> => {
      ...
      const rows = await searchWithinClient(args.clientId, `%${q}%`)
  ...
  ipcMain.handle(
    'search:globalSearch',
    async (_e, query: string): Promise<GlobalSearchResult[]> => {
      const q = query.trim()
      if (q.length < 2) return []
      const db = getDb()
      const pattern = `%${q}%`
      ...
      for (const client of activeClients) {
        if (client.fullName.toLowerCase().includes(q.toLowerCase())) {
          results.push({
            kind: 'client',
            id: client.id,
        ...
        const rows = await searchWithinClient(client.id, pattern)
        for (const row of rows) {
          results.push({ ...row, clientId: client.id, clientName: client.fullName })
          if (results.length >= GLOBAL_RESULT_LIMIT) return results
        }
      }
      return results
    }
